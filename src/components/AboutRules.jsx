...
function AboutRules() {
  ...
  const pieceRules = {
    ...
    {id: 10, pieceName: ship, info: "This is a hybird of a knight and rook.  To move, it must go 1 square horizontally, but has no restriction on vertical movement.  This has almost the same power as a rook.", symbol: "⚓"}
    {id: 11, pieceName: gryphon, info: "This is a super-ship.  It moves like the ship, however, it can do it in the horizontal axis as well.  This piece has almost the same power as a queen.", symbol: "S⚓S"}
  }

  const addtionalRules = {
    {id: 0, ruleName: "King's Succession", info: "If the king is in checkmate, the game is not over if you have at least one prince not in check.  The player can them swap the positions of the king and prince on their turn.  The player who caused checkmate may then decide to capture or not the prince.  Since each player has two princes, they are essentially given 2 free checkmates in the game."}
    ...
    {id: 5, ruleName: "Ship Promotion", info: "If a ship reaches the opponent's side of the game board, it is promoted to gryphon.  One gryphon has the power to quickly end the game."}
    {id: 6, ruleName: "", info: ""}
  }
